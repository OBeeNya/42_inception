...
wp config create --allow-root --dbname=${DATABASE_NAME} --dbuser=${USERNAME} --dbpass=${PASSWORFD} --dbhost=${DB_HOST} --dbprefix=wp --config-file="${WP_PATH}/wp-config.php" --path="${WP_PATH}"
...
wp user create --allow-root "${USERNAME}" "${EMAIL}" --user-pass=${PASSWORD} --path="${WP_PATH}"
